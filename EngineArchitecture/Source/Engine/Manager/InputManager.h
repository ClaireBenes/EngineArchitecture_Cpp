...
/*
 * Overload of the comparison operator for SDL_MouseButtonEvent to enable usage in a map.
...
/*
 The InputManager class is responsible for handling user input events, including keyboard and mouse input. 
...
class InputManager
{
	std::map<SDL_Keycode, InputEvent*> mInputEvents; // Map for storing keyboard events (key code -> InputEvent).
	std::map<Uint32, InputEvent*> mMouseEvents; // Map for storing mouse events (mouse button -> InputEvent).
