...
/**
 * @brief Vector4 is a 4D vector class used in graphics, math, and color representations.
...
public:
	...
	/**
	 * @brief Computes the 3D cross product of two vectors (ignores w).
	 * @param a First vector.
	 * @param b Second vector.
	...
	/**
	 * @brief Reflects a vector across a normal vector.
	...
	/**
	 * @brief Gets a float pointer to the vector's data.
	 * @return Pointer to the first component (x).
	 */
	const float* GetAsFloatPtr() const;

	/**
	 * @brief Accesses a component by index (0 = x, 1 = y, 2 = z, 3 = w).
