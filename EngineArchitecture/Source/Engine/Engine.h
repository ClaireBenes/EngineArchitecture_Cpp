...
/**
 * @brief Engine is the main class responsible for initializing and running the game.
...
public:
	...
	/**
	 * @brief Initializes the engine components, including the window, renderer, and first scene.
	 * @param pRendererType The type of renderer to use.
	 */
	void Initialize(IRenderer::RendererType pRendererType);
	/**
	 * @brief Starts the main game loop. This function repeatedly updates the game state,
	 * processes input, and renders the game until the game is closed.
	 */
	void Loop();
	/**
	 * @brief Renders the game by calling the renderer to draw all objects to the screen.
	 */
	void Render();
	/**
	 * @brief Updates the game state by calling the current scene's update function.
	 */
	void Update();
	/**
	 * @brief Checks for input events such as keyboard and window events, and processes them.
	 */
	void CheckForInputs();
	/**
	 * @brief Closes the engine, performing necessary cleanup tasks.
	 */
	void Close();
...
private:
	...
	/**
	 * @brief The list of scenes managed by the engine.
	 */
	std::vector<Scene*> mScenes{};
	/**
	 * @brief The window for rendering.
	 */
	Window* mWindow{};
	/**
	 * @brief The renderer used for drawing.
	 */
	IRenderer* mRenderer{}; 
